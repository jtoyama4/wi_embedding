...
def get_alexnet_encoder(pretrained=False, im_grayscale=True, im_gradients=True):
    r"""AlexNet model architecture from the
    `"One weird trick..." <https://arxiv.org/abs/1404.5997>`_ paper.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = AlexNetEncoder(im_grayscale=im_grayscale, im_gradients=im_gradients)
    if pretrained:
        model.load_state_dict(model_zoo.load_url(model_urls['alexnet']))
    return model
    
def get_alexnet_decoder(input_shape, num_classes):
    ...
    """
    print(input_shape)
    model = AlexNetDecoder(input_shape, num_classes)
    return model
